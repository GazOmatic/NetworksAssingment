...
def actionGet(message):  # TODO
    ...
    with open(filename,"rb") as fileToSend:
        ...
        while bytesSent < fileSize:
            ...
            print(remainingBytesToSend)
            amoutToSend = max(remainingBytesToSend, BATCH) # Send the size of the file. If the size of the file is less than the normal bytes size to send then send the byte size
            bytesSent += conn.sendfile(fileToSend,bytesSent,amoutToSend)
            conn.sendfile(fileToSend)
            
    print(conn.recv(BATCH))
